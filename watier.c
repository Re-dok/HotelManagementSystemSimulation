...
int main() {
    ...
    if ((key = ftok(SHARED_MEM_KEY_FILE, waiter_id*100)) == -1) {
        perror("ftok");
        exit(1);
    }
   ...
   int shmid_hm;
   //key for manager
if ((key_hm = ftok(SHARED_MEM_KEY_FILE_2,( waiter_id)*100)) == -1) {
        perror("ftok");
        exit(1);
    }

    // Create shared memory segment for manager data
    if ((shmid_hm = shmget(key_hm, sizeof(billData), IPC_CREAT | 0666)) == -1) {
        perror("shmget");
        exit(1);
    }

    // Attach shared memory segment for manager data
...
//----
    ...
    //fa
//   sleep(10);
    printf("Total tables: %d\n", shared_table2->totalAvailableTables);

    // Read and display orders for each table
    printf("\nOrders received from Table %d:\n",waiter_id);
    printf("Table ID: %d\n",waiter_id);
    printf("Number of Customers: %d\n", shared_table->number_of_customers);
    printf("hi\n");
    // printf("Bill amount for the table %d is %d.",waiter_id,shared_table->bill);
    shared_table->bill=0;
    for(int i=1;i<10;i++)
    {
